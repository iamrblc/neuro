...
while startup == 0
    ...
    subjnum=input('V.Sz. sz�ma (eg�sz sz�m < 150)\n');
    if ~isnumeric(subjnum)
        disp('Sz�mot �rj be!')
        continue
    end
    if mod(subjnum,1)~=0
        disp('Eg�sz sz�mot �rj be!')
        continue
    end
    if subjnum < 0
        disp('Sz�m > 0!')
    ...
    if exist([num2str(subjnum) '_log.mat'],'file')
        while ~ismember(resp,[1 2]) 
            resp=input('V.Sz. l�tezik. 1=Folytat (utols� teljes blokkt�l) 2=�j sz�m\n');
        end
        switch resp
            case 1
...
pahandle = PsychPortAudio('Open', 3 , 1, [], fs, 2, [], [], [0 1]); % 'MME' hangsz�r�
